...
if (isset($_GET['id'])) {
    ...
    $pharmacy = mysqli_fetch_assoc($res);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!is_numeric($latitude) || !is_numeric($longitude)) {
    ...
    } else if ($error == '' && isset($_POST['pharmacyId']) && $_POST['pharmacyId'] !== '') {
        // Update
        ...
        if (mysqli_query($conn, $sql)) {
            header("Location: managepharmacy(Pharmacy).php?success=updated");
            exit();
        } else {
        ...
        // Insert
        $result = mysqli_query($conn, "SELECT MAX(pharmacyId) AS maxId FROM pharmacy");
        $row = mysqli_fetch_assoc($result);
        $pharmacyId = ($row['maxId'] ?? 0) + 1;
        $sql = "INSERT INTO pharmacy
                    (pharmacyName, product, message, address, latitude, longitude, phoneNum, opHourStart, opHourEnd, pharmacyImage, userId)
                VALUES
                    ('$name', '$product', '$message', '$address', '$latitude', '$longitude', '$phoneNum', '$opHourStart', '$opHourEnd', '$pharmacyImage', '$userId')";

        if (!mysqli_query($conn, $sql)) {
            die("MySQL Error: " . mysqli_error($conn));
        } else {
            header("Location: managepharmacy(Pharmacy).php?success=added");
            exit();
        }
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="healthcareform(Pharmacy).css">
</head>
<body>
    <div class = "container">
    ...
    <div class="admin-content">
        ...
        <form method="POST" class="admin-form" enctype="multipart/form-data">
            ...
            <div class="form-actions">
                ...
                <a href="managepharmacy(Pharmacy).php" class="btn-cancel">Cancel</a>
            </div>
        </form>
    </div>
    </div>
