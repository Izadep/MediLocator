...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!is_numeric($latitude) || !is_numeric($longitude)) {
        ...
        if (mysqli_query($conn, $sql)) {
            header("Location: manageclinic(Clinic).php?success=updated");
            exit();
        } else {
        ...
        $sql = "INSERT INTO clinic
                    (clinicId, clinicName, specialty, specialServices, message, address, latitude, longitude, phoneNum, opHourStart, opHourEnd, clinicImage, userId)
                VALUES
                    ('$clinicId', '$name', '$specialty', '$specialServices', '$message', '$address', '$latitude', '$longitude', '$phoneNum', '$opHourStart', '$opHourEnd', '$clinicImage', '$userId')";

        if (!mysqli_query($conn, $sql)) {
            die("MySQL Error: " . mysqli_error($conn));
        } else {
            header("Location: manageclinic(Clinic).php?success=added");
            exit();
        }
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="healthcareform(Clinic).css">
</head>
<body>
    <div class = "container">
    ...
    <div class="admin-content">
        ...
        <form method="POST" class="admin-form" enctype="multipart/form-data">
            ...
            <div class="form-actions">
                ...
                <a href="manageclinic(Clinic).php" class="btn-cancel">Cancel</a>
            </div>
        </form>
    </div>
    </div>
